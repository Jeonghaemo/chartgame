    ...
    <html lang="ko">
      ...
      <body className={`${notoSans.className} min-h-screen bg-slate-50 text-slate-900`}>
        ...
        <header className="h-20 border-b bg-white flex items-center">
          ...
          <div className="max-w-[1200px] mx-auto w-full px-4 grid grid-cols-[1fr_auto_1fr] items-center">
            {/* 왼쪽 공간 (로고 등 필요 시 추가 가능) */}
            <div />
            ...
            {/* 오른쪽 로그인/로그아웃 */}
            <div className="justify-self-end flex items-center gap-3 text-base">
              {session?.user ? (
                <form
                  action={async () => {
                    "use server";
